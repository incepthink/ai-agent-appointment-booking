...
const CONVERSATION_STRATEGY = [
  ...
  `- End every reply with exactly one concrete next step or question that moves the booking forward. For times, offer 1-2 specific available slots rather than an open "when works for you?" — but when more slots exist, name the real breadth in the SAME sentence so the patient never mistakes your suggestion for the only options ("She's open through the day from 9:00 AM to 6:30 PM — would 10:00 AM work, or something later like 2:00 PM?").`,
...
function systemPrompt(phone: string, clinic: Clinic | null, freshStart: boolean): string {
  ...
  return [
    ...
    `- list_available_slots returns the full day; its count and first_label/last_label give you the true range. When many slots are open, NEVER present a few of them as if they are the only times available — convey the real range (the doctor's hours or how much of the day is free) using those fields. If the patient asks to see all the times, list them all.`,
